...
import { Lock, ArrowRight, Download, ShieldCheck } from "lucide-react";
  ...
  return (
    ...
    >
      <GlassPanel variant="elevated" className="p-6 sm:p-8">
        ...
        {teaser.coverage ? (
          <div className="mt-4 flex items-start gap-3 rounded-xl border border-border-subtle bg-bg-secondary/30 p-3">
            <ShieldCheck size={16} className="mt-0.5 text-brand-cyan" />
            <div>
              <p className="text-xs font-medium capitalize text-text-primary">
                {teaser.coverage.status} evidence coverage ·{" "}
                {teaser.coverage.score}%
              </p>
              <p className="mt-1 text-xs text-text-muted">
                {teaser.coverage.evidenceCount} normalized sources
                {teaser.confidence
                  ? ` · ${teaser.confidence.level} confidence`
                  : ""}
              </p>
            </div>
          </div>
        ...
        {teaser.quickWins.length > 0 ? (
          <div className="mt-6">
            <h3 className="text-sm font-semibold text-text-primary">Quick wins</h3>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-text-muted">
              Sign in to access the full evidence, implementation blueprint,
              and PDF.
              {email ? (
                <>
                  {" "}
...
function MiniList({ title, items }: { title: string; items: string[] }) {
  return (
    <div className="rounded-xl border border-border-subtle/60 bg-bg-secondary/30 p-3">
      ...
      {items.length === 0 ? (
        <p className="mt-2 text-xs text-text-muted">Unlock full detail</p>
      ) : (
